...
/*** DOCUMENTATION
    ...
    <manager name="QueueChangePositionCaller" language="en_US">
        <synopsis>
            Change position of a caller on queue.
        </synopsis>
        <syntax>
			<xi:include xpointer="xpointer(/docs/manager[@name='Login']/syntax/parameter[@name='ActionID'])" />
			<parameter name="Queue" required="true">
			...
			<parameter name="Position" required="true">
				<para>Position value for change for caller on queue.</para>
			</parameter>
		</syntax>
		<description>
...
struct penalty_rule {
    ...
    AST_LIST_ENTRY(penalty_rule)
    list;  /*!< Next penalty_rule */
};
...
struct call_queue {
    ...
    AST_LIST_ENTRY(call_queue)
    list;    /*!< Next call queue */
    AST_LIST_HEAD_NOLOCK(, penalty_rule)
    rules; /*!< The list of penalty rules to invoke */
};

struct rule_list {
    char name[80];
    AST_LIST_HEAD_NOLOCK(,penalty_rule) rules;
    AST_LIST_ENTRY(rule_list)
    list;
};
...
static void set_queue_variables(struct call_queue *q, struct ast_channel *chan) {
    ...
    if (q->setqueuevar) {
        ...
        snprintf(interfacevar, sizeof(interfacevar),
                 ...
                 q->callsabandoned, q->servicelevel, sl);
...
static struct ast_manager_event_blob *queue_channel_to_ami(const char *type, struct stasis_message *message) {
    ...
    RAII_VAR(
    struct ast_str *, channel_string, NULL, ast_free);
    RAII_VAR(
    struct ast_str *, event_string, NULL, ast_free);
...
static struct ast_manager_event_blob *queue_multi_channel_to_ami(const char *type, struct stasis_message *message) {
    ...
    RAII_VAR(
    struct ast_str *, caller_event_string, NULL, ast_free);
    RAII_VAR(
    struct ast_str *, agent_event_string, NULL, ast_free);
    RAII_VAR(
    struct ast_str *, event_string, NULL, ast_free);
...
static void queue_publish_multi_channel_snapshot_blob(struct stasis_topic *topic,
    ...
    RAII_VAR(
    struct ast_multi_channel_blob *, payload, NULL, ao2_cleanup);
    RAII_VAR(
    struct stasis_message *, msg, NULL, ao2_cleanup);
...
static void queue_publish_multi_channel_blob(struct ast_channel *caller, struct ast_channel *agent,
                                             struct stasis_message_type *type, struct ast_json *blob) {
    RAII_VAR(
    struct ast_channel_snapshot *, caller_snapshot, NULL, ao2_cleanup);
    RAII_VAR(
    struct ast_channel_snapshot *, agent_snapshot, NULL, ao2_cleanup);
...
static void queue_publish_member_blob(struct stasis_message_type *type, struct ast_json *blob) {
    RAII_VAR(
    struct ast_json_payload *, payload, NULL, ao2_cleanup);
    RAII_VAR(
    struct stasis_message *, msg, NULL, ao2_cleanup);
 ...
 * then -1 is returned.
 */
static int get_member_status(struct call_queue *q, int max_penalty, int min_penalty, int raise_penalty,
                             enum empty_conditions conditions, int devstate) {
    struct member *member;
...
static int get_queue_member_status(struct member *cur) {
    return ast_strlen_zero(cur->state_exten) ? ast_device_state(cur->state_interface) : extensionstate2devicestate(
            ast_extension_state(NULL, cur->state_context, cur->state_exten));
}
...
create_queue_member(const char *interface, const char *membername, int penalty, int paused, const char *state_interface,
                    int ringinuse, int wrapuptime) {
    struct member *cur;
...
static void member_remove_from_queue(struct call_queue *queue, struct member *mem) {
    ...
    ao2_lock(queue->members);
    ast_devstate_changed(QUEUE_UNKNOWN_PAUSED_DEVSTATE, AST_DEVSTATE_CACHABLE, "Queue:%s_pause_%s", queue->name,
                         mem->interface);
...
static struct call_queue *find_load_queue_rt_friendly(const char *queuename) {
    ...
    if (!q || q->realtime) {
        ...
        if (queue_vars) {
            member_config = ast_load_realtime_multientry("queue_members", "interface LIKE", "%", "queue_name",
                                                         queuename, SENTINEL);
            if (!member_config) {
...
static int
update_realtime_member_field(struct member *mem, const char *queue_name, const char *field, const char *value) {
    int ret = -1;
...
static void update_realtime_members(struct call_queue *q) {
    ...
    if (!(member_config = ast_load_realtime_multientry("queue_members", "interface LIKE", "%", "queue_name", q->name,
                                                       SENTINEL))) {
...
static int join_queue(char *queuename, struct queue_ent *qe, enum queue_result *reason, int position) {
    ...
    if (*reason == QUEUE_UNKNOWN && q->maxlen && (q->count >= q->maxlen)) {
        *reason = QUEUE_FULL;
        ...
        RAII_VAR(
        struct ast_json *, blob, NULL, ast_json_unref);
...
static int valid_exit(struct queue_ent *qe, char digit) {
    ...
    if (!ast_canmatch_extension(qe->chan, qe->context, qe->digits, 1,
                                S_COR(ast_channel_caller(qe->chan)->id.number.valid,
                                      ast_channel_caller(qe->chan)->id.number.str, NULL))) {
        qe->digits[0] = '\0';
        return 0;
    }
...
static int say_position(struct queue_ent *qe, int ringing) {
    ...
    if (announceposition == 1) {
        ...
        } else if (qe->parent->announceposition == ANNOUNCEPOSITION_MORE_THAN &&
                   qe->pos > qe->parent->announcepositionlimit) {
            res = (
    ...
    if (qe->parent->roundingseconds) {
        avgholdsecs = (labs(((qe->parent->holdtime + 30) - (now - qe->start))) - 60 * avgholdmins) /
...
static void publish_dial_end_event(struct ast_channel *in, struct callattempt *outgoing, struct ast_channel *exception,
                                   const char *status) {
...
static void hangupcalls(struct queue_ent *qe, struct callattempt *outgoing, struct ast_channel *exception,
...
static int ring_entry(struct queue_ent *qe, struct callattempt *tmp, int *busies) {
    ...
    if (!ast_channel_caller(tmp->chan)->id.number.valid) {
        if (ast_channel_connected(qe->chan)->id.number.valid) {
            ...
            ast_set_callerid(tmp->chan, S_OR(ast_channel_macroexten(qe->chan), ast_channel_exten(qe->chan)), NULL,
                             NULL);
        }
    ...
    ast_channel_dialcontext_set(tmp->chan,
...
static void
rna(int rnatime, struct queue_ent *qe, struct ast_channel *peer, char *interface, char *membername, int autopause) {
    RAII_VAR(
    struct ast_json *, blob, NULL, ast_json_unref);
 ...
 */
static struct callattempt *
wait_for_answer(struct queue_ent *qe, struct callattempt *outgoing, int *to, char *digit, int prebusies,
    ...
    while ((*to = ast_remaining_ms(start_time_tv, orig)) && !peer) {
                ...
                if (!ast_strlen_zero(ast_channel_call_forward(o->chan)) && !forwardsallowed) {
                    ...
                    if ((stuff = strchr(tmpchan, '/'))) {
                        *stuff++ = '\0';
                        ...
                        snprintf(tmpchan, sizeof(tmpchan), "%s@%s", ast_channel_call_forward(o->chan),
                                 forward_context ? forward_context : ast_channel_context(o->chan));
                        ast_channel_unlock(o->chan);
                        stuff = tmpchan;
                        tech = "Local";
                    }
                    ...
                    if (!o->chan) {
                        ...
                        ast_channel_dialed(o->chan)->transit_network_select = ast_channel_dialed(
                                in)->transit_network_select;
        ...
        if (winner == in) {
            ...
            if (canceled_by_caller) {
                ...
                for (o = start; o; o = o->call_next) {
                    if (o->chan) {
                        ast_queue_log(qe->parent->name, ast_channel_uniqueid(qe->chan), o->member->membername,
                                      "RINGCANCELED", "%d", (int) ast_tvdiff_ms(ast_tvnow(), start_time_tv));
                    }
                }
                return NULL;
            }
...
static int wait_our_turn(struct queue_ent *qe, int ringing, enum queue_result *reason) {
    ...
    for (;;) {
        ...
        if (qe->parent->leavewhenempty) {
            ...
            if ((status = get_member_status(qe->parent, qe->max_penalty, qe->min_penalty, qe->raise_penalty,
                                            qe->parent->leavewhenempty, 0))) {
                *reason = QUEUE_LEAVEEMPTY;
                ast_queue_log(qe->parent->name, ast_channel_uniqueid(qe->chan), "NONE", "EXITEMPTY", "%d|%d|%ld",
 ...
 */
static int
calc_metric(struct call_queue *q, struct member *mem, int pos, struct queue_ent *qe, struct callattempt *tmp) {
...
static void log_attended_transfer(struct queue_stasis_data *queue_data,
    ...
    RAII_VAR(
    struct ast_str *, transfer_str, ast_str_create(32), ast_free);
...
static void handle_blind_transfer(void *userdata, struct stasis_subscription *sub,
    ...
    struct ast_channel_snapshot *, caller_snapshot, NULL, ao2_cleanup);
    RAII_VAR(
    struct ast_channel_snapshot *, member_snapshot, NULL, ao2_cleanup);
...
static void handle_hangup(void *userdata, struct stasis_subscription *sub,
    ...
    struct ast_channel_snapshot *, member_snapshot, NULL, ao2_cleanup);
    RAII_VAR(
    struct ast_channel *, chan, NULL, ao2_cleanup);
...
static int setup_stasis_subs(struct queue_ent *qe, struct ast_channel *peer, struct member *mem,
    ...
    struct queue_stasis_data *queue_data = queue_stasis_data_alloc(qe, peer, mem, holdstart, starttime,
                                                                   callcompletedinsl);
...
static void end_bridge_callback_data_fixup(struct ast_bridge_config *bconfig, struct ast_channel *originator,
 ...
 * \return Nothing
 */
static void setup_peer_after_bridge_goto(struct ast_channel *chan, struct ast_channel *peer, struct ast_flags *opts,
                                         char *opt_args[]) {
    const char *context;
 ...
 */
static int
try_calling(struct queue_ent *qe, struct ast_flags opts, char **opt_args, char *announceoverride, const char *url,
            int *tries, int *noption, const char *agi, const char *macro, const char *gosub, int ringing) {
    struct member *cur;
	...
	*/
    ...
    if (!peer) {
        ...
        struct ast_json *, blob, NULL, ast_json_unref);
        RAII_VAR(
        struct ast_str *, interfacevar, ast_str_create(325), ast_free);
        ...
        if (announce || qe->parent->reportholdtime || qe->parent->memberdelay) {
            ...
            if (ast_check_hangup(peer)) {
                ...
                ast_log(LOG_NOTICE, "Caller was about to talk to agent on %s but the caller hungup.\n",
                        ast_channel_name(peer));
                ast_queue_log(queuename, ast_channel_uniqueid(qe->chan), member->membername, "ABANDON", "%d|%d|%ld",
                              qe->pos, qe->opos, (long) (time(NULL) - qe->start));
                record_abandoned(qe);
                qe->handled = -1;
                ast_channel_publish_dial(qe->chan, peer, member->interface,
        ...
        if (qe->parent->setinterfacevar && interfacevar) {
            ast_str_set(&interfacevar, 0,
                        "MEMBERINTERFACE=%s,MEMBERNAME=%s,MEMBERCALLS=%d,MEMBERLASTCALL=%ld,MEMBERPENALTY=%d,MEMBERDYNAMIC=%d,MEMBERREALTIME=%d",
                        member->interface, member->membername, member->calls, (long) member->lastcall, member->penalty,
                        member->dynamic, member->realtime);
        ...
        if (*qe->parent->monfmt) {
            if (!qe->parent->montype) {
                ...
                ast_channel_lock(qe->chan);
                if ((monexec = pbx_builtin_getvar_helper(qe->chan, "MONITOR_EXEC")) ||
                ...
                if (monitorfilename) {
                    ...
                    ast_monitor_start(which, qe->parent->monfmt, ast_channel_uniqueid(qe->chan), 1,
                                      X_REC_IN | X_REC_OUT, NULL);
                } else {
        ...
        if (!ast_strlen_zero(gosubexec)) {
            ...
            if (gosub_argstart) {
                ...
                *gosub_argstart = 0;
                if (!ast_exists_extension(peer, gosubexec, "s", 1, S_COR(ast_channel_caller(peer)->id.number.valid,
                                                                         ast_channel_caller(peer)->id.number.str,
                                                                         NULL)) &&
                    ast_exists_extension(peer, gosubexec, "~~s~~", 1, S_COR(ast_channel_caller(peer)->id.number.valid,
                                                                            ast_channel_caller(peer)->id.number.str,
                                                                            NULL))) {
                    what_is_s = "~~s~~";
                }
        ...
        ast_queue_log(queuename, ast_channel_uniqueid(qe->chan), member->membername, "CONNECT", "%ld|%s|%ld",
                      (long) (time(NULL) - qe->start), ast_channel_uniqueid(peer),
        ...
        bridge = ast_bridge_call_with_flags(qe->chan, peer, &bridge_config,
                                            AST_BRIDGE_FLAG_MERGE_INHIBIT_FROM | AST_BRIDGE_FLAG_MERGE_INHIBIT_TO |
...
*/
static int
add_to_queue(const char *queuename, const char *interface, const char *membername, int penalty, int paused, int dump,
             const char *state_interface, const char *reason_paused, int wrapuptime) {
    struct call_queue *q;
 ...
 * \retval RES_NOT_CALLER queue exists but no caller
 * \retval RES_POSITION_TOO_BIG queue exists, caller exists, but position is too big
 * \retval RES_POSITION_TOO_SMALL queue exists, caller exists, but position is too small
*/

static int change_position_caller_on_queue(const char *queuename, const char *caller, int position) {
    struct call_queue *q;
    struct queue_ent *qe;
    struct queue_ent *queueEnt = NULL;
    int res = RES_OKAY;
    ...
    if (position > q->maxlen) {
        res = RES_POSITION_TOO_BIG;
        return res;
    }
...
static void reload_queue_members(void) {
    ...
    for (entry = db_tree; entry; entry = entry->next) {
        ...
        while ((member = strsep(&cur_ptr, ",|"))) {
            ...
            if (add_to_queue(queue_name, interface, membername, penalty, paused, 0, state_interface, reason_paused,
                             wrapuptime) == RES_OUTOFMEMORY) {
...
static int aqm_exec(struct ast_channel *chan, const char *data) {
    ...
    switch (add_to_queue(args.queuename, args.interface, args.membername, penalty, 0, queue_persistent_members,
                         args.state_interface, NULL, wrapuptime)) {
        case RES_OKAY:
...
static int queue_exec(struct ast_channel *chan, const char *data) {
    ...
    for (;;) {
        ...
        res = try_calling(&qe, opts, opt_args, args.announceoverride, args.url, &tries, &noption, args.agi, args.macro,
                          args.gosub, ringing);
        if (res) {
            goto stop;
        }
        ...
        if (qe.parent->leavewhenempty) {
            ...
            if ((status = get_member_status(qe.parent, qe.max_penalty, qe.min_penalty, qe.raise_penalty,
                ...
                record_abandoned(&qe);
                reason = QUEUE_LEAVEEMPTY;
                ast_queue_log(args.queuename, ast_channel_uniqueid(chan), "NONE", "EXITEMPTY", "%d|%d|%ld", qe.pos,
        ...
        if (qe.expire && (time(NULL) >= qe.expire)) {
            ...
            reason = QUEUE_TIMEOUT;
            res = 0;
            ast_queue_log(qe.parent->name, ast_channel_uniqueid(qe.chan), "NONE", "EXITWITHTIMEOUT", "%d|%d|%ld",
                          qe.pos, qe.opos, (long) (time(NULL) - qe.start));
            break;
        }
    ...
    if ((q = find_load_queue_rt_friendly(data))) {
        ...
        if (q->setqueuevar) {
            ...
            snprintf(interfacevar, sizeof(interfacevar),
                     ...
                     q->maxlen, int2strat(q->strategy), q->count, q->holdtime, q->talktime, q->callscompleted,
                     q->callsabandoned, q->servicelevel, sl);
...
static int
queue_function_queuegetchannel(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len) {
    int position;
    char *parse;
    struct call_queue *q;
    struct ast_variable *var;
...
static int
queue_function_queuewaitingcount(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len) {
    int count = 0;
    struct call_queue *q, tmpq = {
            .name = data,
    };
...
static int
queue_function_queuememberlist(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len) {
    struct call_queue *q;
    struct member *m;
...
static int
queue_function_memberpenalty_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len) {
    int penalty;
    AST_DECLARE_APP_ARGS(args,
                         AST_APP_ARG(queuename);
    AST_APP_ARG(interface);
...
static int
queue_function_memberpenalty_write(struct ast_channel *chan, const char *cmd, char *data, const char *value) {
    int penalty;
    AST_DECLARE_APP_ARGS(args,
                         AST_APP_ARG(queuename);
    AST_APP_ARG(interface);
...
static char *__queues_show(struct mansession *s, int fd, int argc, const char *const *argv) {
    ...
    while ((q = ao2_t_iterator_next(&queue_iter, "Iterate through queues"))) {
        ...
        if (q->callscompleted + q->callsabandoned > 0) {
            sl2 = 100 * (((float) q->callsabandonedinsl + (float) q->callscompletedinsl) /
                         ((float) q->callsabandoned + (float) q->callscompleted));
        }

        ast_str_append(&out, 0,
                       ") in '%s' strategy (%ds holdtime, %ds talktime), W:%d, C:%d, A:%d, SL:%2.1f%%, SL2:%2.1f%% within %ds",
                       int2strat(q->strategy), q->holdtime, q->talktime, q->weight, q->callscompleted,
                       q->callsabandoned, sl, sl2, q->servicelevel);
        ...
        if (!ao2_container_count(q->members)) {
            ...
            while ((mem = ao2_iterator_next(&mem_iter))) {
                ...
                ast_str_append(&out, 0, "%s%s%s%s%s%s%s%s%s",
                               mem->dynamic ? ast_term_color(COLOR_CYAN, COLOR_BLACK) : "",
                               mem->dynamic ? " (dynamic)" : "", ast_term_reset(),
                               mem->realtime ? ast_term_color(COLOR_MAGENTA, COLOR_BLACK) : "",
                               mem->realtime ? " (realtime)" : "", ast_term_reset(),
                               mem->starttime ? ast_term_color(COLOR_BROWN, COLOR_BLACK) : "",
                               mem->starttime ? " (in call)" : "", ast_term_reset());
        ...
        if (ast_strlen_zero(rule) || !strcasecmp(rule, rl_iter->name)) {
            ...
            AST_LIST_TRAVERSE(&rl_iter->rules, pr_iter, list)
            {
                astman_append(s, "Rule: %d,%s%d,%s%d\r\n", pr_iter->time,
                              pr_iter->max_relative && pr_iter->max_value >= 0 ? "+" : "", pr_iter->max_value,
                              pr_iter->min_relative && pr_iter->min_value >= 0 ? "+" : "", pr_iter->min_value);
            }
    ...
    while ((q = ao2_t_iterator_next(&queue_iter, "Iterate through queues"))) {
        ...
        if (ast_strlen_zero(queuefilter) || !strcasecmp(q->name, queuefilter)) {
            ...
            astman_append(s, "Event: QueueSummary\r\n"
                             ...
                             "%s"
                             "\r\n",
                          q->name, qmemcount, qmemavail, qchancount, q->holdtime, q->talktime, qlongestholdtime,
                          idText);
            ++qsummaries;
        }
        ao2_unlock(q);
...
static int manager_queues_status(struct mansession *s, const struct message *m) {
    ...
    while ((q = ao2_t_iterator_next(&queue_iter, "Iterate through queues"))) {
        ...
        if (ast_strlen_zero(queuefilter) || !strcasecmp(q->name, queuefilter)) {
            ...
            sl2 = (((q->callscompleted + q->callsabandoned) > 0) ? 100 * (((float) q->callsabandonedinsl +
                                                                           (float) q->callscompletedinsl) /
                                                                          ((float) q->callsabandoned +
                                                                           (float) q->callscompleted)) : 0);
            ...
            astman_append(s, "Event: QueueParams\r\n"
                             ...
                             "%s"
                             "\r\n",
                          q->name, q->maxlen, int2strat(q->strategy), q->count, q->holdtime, q->talktime,
                          q->callscompleted,
            ...
            while ((mem = ao2_iterator_next(&mem_iter))) {
                if (ast_strlen_zero(memberfilter) || !strcmp(mem->interface, memberfilter) ||
                    !strcmp(mem->membername, memberfilter)) {
                    astman_append(s, "Event: QueueMember\r\n"
                                     ...
                                     "%s"
                                     "\r\n",
                                  q->name, mem->membername, mem->interface, mem->state_interface,
                                  mem->dynamic ? "dynamic" : "static",
                                  mem->penalty, mem->calls, (int) mem->lastcall, (int) mem->lastpause,
                                  mem->starttime ? 1 : 0, mem->status,
            ...
            for (qe = q->head; qe; qe = qe->next) {
                astman_append(s, "Event: QueueEntry\r\n"
                              ...
                              S_COR(ast_channel_caller(qe->chan)->id.number.valid,
                                    ast_channel_caller(qe->chan)->id.number.str, "unknown"),
                              S_COR(ast_channel_caller(qe->chan)->id.name.valid,
                                    ast_channel_caller(qe->chan)->id.name.str, "unknown"),
                              S_COR(ast_channel_connected(qe->chan)->id.number.valid,
                                    ast_channel_connected(qe->chan)->id.number.str, "unknown"),
                              S_COR(ast_channel_connected(qe->chan)->id.name.valid,
                                    ast_channel_connected(qe->chan)->id.name.str, "unknown"),
...
static int manager_change_position_caller_on_queue(struct mansession *s, const struct message *m) {
    ...
    queuename = astman_get_header(m, "Queue");
    caller = astman_get_header(m, "Caller");
    position_s = astman_get_header(m, "Position");
    ...
    if (ast_strlen_zero(position_s)) {
        astman_send_error(s, m, "'Position' not specified.");
        return 0;
    } else if (sscanf(position_s, "%30d", &position) != 1) {
        astman_send_error(s, m, "'Position' need integer.");
        return 0;
    }

    switch (change_position_caller_on_queue(queuename, caller, position)) {
        case RES_OKAY:
            astman_send_ack(s, m, "Position change for caller on queue");
            break;
        case RES_NOSUCHQUEUE:
            astman_send_error(s, m, "Unable to change position caller on queue: No such queue");
            break;
        case RES_NOT_CALLER:
            astman_send_error(s, m, "Unable to change position caller on queue: No such caller");
            break;
        case RES_POSITION_TOO_BIG:
            astman_send_error(s, m, "Unable to change position caller on queue: Position too big");
            break;
        case RES_POSITION_TOO_SMALL:
            astman_send_error(s, m, "Unable to change position caller on queue: Position too small");
            break;
    }
...
static char *handle_queue_change_position_caller(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
    ...
    if (a->argc != 8) {
        ...
        return CLI_SHOWUSAGE;
    } else if (sscanf(a->argv[7], "%30d", &position) != 1) {
        ast_log(LOG_ERROR, "<position> parameter must be an integer.\n");
        return CLI_SHOWUSAGE;
    }
    ...
    switch (change_position_caller_on_queue(queuename, caller, position)) {
        case RES_OKAY:
            ...
            break;
        case RES_NOSUCHQUEUE:
            ast_cli(a->fd, "Unable to change position caller %s on queue '%s': No such queue\n", caller, queuename);
            break;
        case RES_NOT_CALLER:
...
static char *handle_queue_change_priority_caller(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
    ...
    switch (change_priority_caller_on_queue(queuename, caller, priority)) {
        case RES_OKAY:
            ...
            break;
        case RES_NOSUCHQUEUE:
            ast_cli(a->fd, "Unable to change priority caller %s on queue '%s': No such queue\n", caller, queuename);
            break;
        case RES_NOT_CALLER:
        ...
        if (ast_strlen_zero(rule) || !strcasecmp(rl_iter->name, rule)) {
            ...
            AST_LIST_TRAVERSE(&rl_iter->rules, pr_iter, list)
            {
                ast_cli(a->fd,
                        ...
                        pr_iter->time, pr_iter->max_relative ? "by" : "to", pr_iter->max_value,
                        pr_iter->min_relative ? "by" : "to", pr_iter->min_value, pr_iter->raise_relative ? "by" : "to",
                        pr_iter->raise_value);
            }
        }
    }
...
static int qupd_exec(struct ast_channel *chan, const char *data) {
    ...
    if (ast_strlen_zero(args.queuename) || ast_strlen_zero(args.uniqueid) || ast_strlen_zero(args.agent) ||
        ast_strlen_zero(args.status)) {
        ast_log(LOG_WARNING,
    ...
    if (q->members) {
        ...
        while ((mem = ao2_iterator_next(&mem_iter))) {
            if (!strcasecmp(mem->membername, args.agent)) {
                ...
                ast_queue_log(args.queuename, args.uniqueid, args.agent, "OUTCALL", "%s|%s|%s", args.status,
                              args.talktime, args.params);
            }
...
static int load_module(void) {
    ...
    pending_members = ao2_container_alloc_hash(AO2_ALLOC_OPT_LOCK_MUTEX, 0,
                                               MAX_CALL_ATTEMPT_BUCKETS, pending_members_hash, NULL,
                                               pending_members_cmp);
    if (!pending_members) {
        unload_module();
        return AST_MODULE_LOAD_DECLINE;
	 ...
	 */
    member_config = ast_load_realtime_multientry("queue_members", "interface LIKE", "%", "queue_name LIKE", "%",
                                                 SENTINEL);
    if (!member_config) {
